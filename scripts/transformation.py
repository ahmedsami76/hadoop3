...
#
"""This module transforms properties into different formats"""  
...
def to_yaml(content):  
  ...
  keys = list(props.keys())  # Ensure keys is a list for Python 3  
  yaml_props = {}  
  for key in keys:  
    parts = key.split(".")  
    node = yaml_props  
    ...
    for part in parts[:-1]:  
      if part.isdigit():  
        part = int(part)  # Convert to int for indexing  
      ...
      prev_part = part  
    if parts[-1].isdigit():  
      parts[-1] = int(parts[-1])  # Convert to int for indexing  
      if isinstance(node, dict):  
        parent_node[prev_part] = []  
        node = parent_node[prev_part]  
      while len(node) <= parts[-1]:  
        node.append(None)  
...
def to_properties(content):  
  ...
  result = ""  
  props = process_properties(content)  
  for key, val in props.items():  # Use .items() for Python 3  
    result += "{}: {}\n".format(key, val)  
  return result  
